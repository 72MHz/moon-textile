...
class VariantRadiosQuickEdit extends SwatchInit {
  ...
  updateMedia() {
    ...
    const imageSrc = this.getVariantImageSrc(this.currentVariant);
    if (!imageSrc) return;
    if (form.querySelector(".product__media img")) {
      form.querySelector(".product__media img").removeAttribute("srcset");
      form.querySelector(".product__media img").setAttribute("src", imageSrc);
    }
  }
...
class VariantRadiosBundle extends SwatchInit {
  ...
  async updateMedia() {
    ...
    const activeSwatch = this.productTarget.querySelector(
      ".product__color-swatches--js.active"
    );
    const imageSrc = this.getVariantImageSrc(
      this.currentVariant,
      activeSwatch
    );
    if (!imageSrc) return;

    const img = this.productTarget.querySelector(".product__media img");
    if (!img) return;
    ...
    const secondaryImg = this.productTarget.querySelector(
      ".product__media .secondary-image"
    );
    if (secondaryImg) {
    ...
    await Motion.animate(img, { opacity: [1, 0] }, { duration: 0.1, easing: "ease-in" });
    ...
    Motion.animate(img, { opacity: [0, 1] }, { duration: 0.1, easing: "ease-in" });
  }
...
class SwatchFunctions extends SwatchInit {
  ...
  updateMedia(variantGroup = false) {
    ...
    const mediaId = this.getVariantMediaId(this.currentVariant);
    const imageSrc = this.getVariantImageSrc(this.currentVariant);

    if (!layout) {
      const galleryImg = this.productTarget.querySelector(
        ".media-main-swiper img, .product__media img"
      );
      if (galleryImg && imageSrc) {
        galleryImg.removeAttribute("srcset");
        galleryImg.removeAttribute("sizes");
        galleryImg.setAttribute("src", imageSrc);
      }
      return;
    }
    ...
    if (layout === "thumbnail") {
      ...
      if (!mediaId) {
        const galleryImg = this.productTarget.querySelector(".media-main-swiper img");
        if (galleryImg && imageSrc) {
          galleryImg.removeAttribute("srcset");
          galleryImg.removeAttribute("sizes");
          galleryImg.setAttribute("src", imageSrc);
      ...
      slide_items.forEach((e, index) => {
        const slideMediaId = e.getAttribute("data-media-id");
        if (
          slideMediaId &&
          (slideMediaId === `${this.dataset.section}-${mediaId}` ||
            slideMediaId === String(mediaId))
        ) {
      ...
      const mediaElement = document.querySelector(
        `[data-media-id="${this.dataset.section}-${mediaId}"], [data-media-id="${mediaId}"]`
      );
      if (mediaElement) {
